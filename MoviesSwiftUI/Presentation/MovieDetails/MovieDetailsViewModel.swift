...
extension MovieDetailViewModel {
    ...
    struct MovieDetail {
        ...
        init(movie: MovieDetailsResponseDTO) {
            id = movie.id
            title = movie.title
            ...
            releasedAt = movie.releaseDate ?? "N/A"
            language = movie.spokenLanguages.first?.name ?? "N/A"
        }
    }
}
...
extension MovieDetailViewModel {
    ...
    static func whenLoading() -> Feedback<State, Event> {
        Feedback { (state: State) -> AnyPublisher<Event, Never> in
            ...
            return MoviesAPI.movieDetails(with: MovieDetailsRequestDTO(id: id))
